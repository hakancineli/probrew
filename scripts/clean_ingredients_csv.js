...
async function main() {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (!usedSet.has(name)) {
            removed.push(name);
            continue; // delete if not used in any recipe
        }
        ...
        kept.push(fixedCols.join(';'));
    }

    const newContent = kept.join('\n');
    fs.writeFileSync(csvPath, newContent, { encoding: 'utf-8' });
    console.log('Cleaned CSV written, rows kept:', kept.length - 1);
    console.log('Rows removed:', removed.length);
    if (removed.length) {
        console.log('Removed ingredients:', removed.join(', '));
    }
}
